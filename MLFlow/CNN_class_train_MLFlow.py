...
def create_model(num_filters=128, dropout_rate=0.6):
    ...
    with mlflow.start_run():
        ...
        if not optimize_hyperparameters:
            ...
            def objective(trial):
                ...
                model = create_model(num_filters=num_filters, dropout_rate=dropout_rate)
                model = assign_custom_weights_to_Conv2D_layer(model)
                ...
                early_stopping = EarlyStopping(monitor='val_loss', patience=4, restore_best_weights=True)
                history = model.fit(train_images_gen, batch_size=32, epochs=15, verbose=1, validation_data=test_images_gen, callbacks=[model_checkpoint, early_stopping, TFKerasPruningCallback(trial, 'val_accuracy')])
...
if __name__ == "__main__":
    train_model(optimize_hyperparameters=False, mlflow_name="no_custom_weights_and_with_optimized_hyperparameters")